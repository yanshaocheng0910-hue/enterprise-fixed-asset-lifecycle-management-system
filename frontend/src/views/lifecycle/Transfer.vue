<template>
  <div>
    ...
    <el-dialog v-model="createVisible" title="新增调拨" width="560px" :close-on-click-modal="false">
...
async function handleSubmitApproval(row: any) {
  try {
    await ElMessageBox.confirm('确认提交此调拨单至审批流程？', '提示', { type: 'warning' })
  } catch { return }
  try {
    const r = await submitApproval({ businessType: 'TRANSFER', businessId: row.id })
    if (r.code === 200) {
